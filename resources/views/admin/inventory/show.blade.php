...
@section('content')
        <!-- Content -->
    <div>
        <div class="bg-white flex-1 px-8">
            <table class="table-auto w-full border border-gray-200">
                ...
                @forelse ($data as $inventory)
                    <tr>
                        ...
                        <td class="border border-gray-200 py-2 text-center">{{ $inventory->nama_barang }}</td>
                        <td class="border border-gray-200 py-2 text-center">{{ $inventory->category->nama_kategori }}</td>
                        <td class="border border-gray-200 py-2 text-center">{{ $inventory->kuota }}</td>
                        <td class="border border-gray-200 py-2 flex justify-center items-center gap-4">
                            <a class="text-blue-600 hover:underline font-bold py-2 px-4 rounded" href="{{ route('admin.inventory.edit', $inventory) }}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                </svg>
                            </a>
                            <form action="{{ route('admin.inventory.delete', $inventory) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus barang ini?');" class="py-2 px-4">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-600 hover:underline font-bold rounded">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
...
<body class="bg-gray-100">
    ...
    <header class="gradient-animation text-white text-center text-2xl font-bold py-4 px-8 sticky top-0 z-50">
        Data Riwayat Peminjaman - ADMIN
    </header>
    <div class="flex h-screen">
        <!-- Sidebar -->
        <div id="sidebar" class="bg-teal-300 w-16 transition-width duration-300 flex flex-col inventorys-center py-6">
            <!-- Toggle Button -->
            ...
            <nav class="space-y-6 w-full">
                <a href="#" class="text-black flex justify-center gap-4 p-2 hover:bg-teal-400 rounded-md">
